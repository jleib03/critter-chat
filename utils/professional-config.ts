...
export const isTimeBlocked = (
  ...
  if (!config?.blockedTimes || !Array.isArray(config.blockedTimes)) {
    return false
  }
  ...
  return config.blockedTimes.some((blockedTime) => {
    try {
      ...
      console.error("Error checking blocked time:", error, blockedTime)
      return false
    }
  })
}
...
export const getEmployeesWorkingAtTime = (
  ...
  return activeEmployees.filter((employee) => {
    try {
      ...
      console.error("Error checking employee availability:", error, employee)
      return false
    }
  })
}
...
export const calculateAvailableSlots = (
  ...
  try {
    ...
    if (employeesWorkingCount === 0 && (!config.employees || config.employees.length === 0)) {
    ...
    if (capacityRules?.maxConcurrentBookings && capacityRules.maxConcurrentBookings > 0) {
    ...
    if (capacityRules?.maxBookingsPerDay && dailyBookings.length >= capacityRules.maxBookingsPerDay) {
      return {
        availableSlots: 0,
        ...
        capacityBreakdown: {
          employeesWorking: employeesWorkingCount,
          capacityLimit: capacityRules.maxConcurrentBookings || 1,
          finalCapacity,
    ...
    if (capacityRules?.bufferTimeBetweenBookings && capacityRules.bufferTimeBetweenBookings > 0) {
