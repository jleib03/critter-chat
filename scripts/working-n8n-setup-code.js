...
// Based on working template pattern

// Declare the $input and $ variables
const $input = require("n8n-core").$input
const $ = require("n8n-core").$
...
allInputs.forEach((input, index) => {
  ...
  } else if (data.professional_id || data.config_data) {
    // This is availability setup data
    availabilitySetupData = data
  }
})

// Try to access data from previous nodes directly (adjust node names to match yours)
try {
  // Access availability setup data
...
try {
  // Access schedule availability data
...
const professionalId = availabilitySetupData?.professional_id || scheduleAvailData?.professional_id || "unknown"
...
// Create the final output using the same pattern as your working template
const output = {
  success: true,
  ...
  // Debug info similar to your working template
  debug_info: {
  ...
  // Raw data for debugging (similar to your template)
  raw_data: {
